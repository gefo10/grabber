...
/**
 * Integration test for ProductController Tests the entire stack: Controller → Service → Repository
 * → Database
 */
@SpringBootTest
...
class ProductControllerIntegrationTest {
  ...
  private RequestPostProcessor asUser() {
    return mockRequest -> {
      mockRequest.addHeader("Authorization", "Bearer " + userToken);
      return mockRequest;
    };
  }
  ...
  private RequestPostProcessor asAdmin() {
    return mockRequest -> {
      mockRequest.addHeader("Authorization", "Bearer " + adminToken);
      return mockRequest;
    };
  }
  ...
  void getProductById_ExistingProduct_ReturnsProduct() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/products/{productId}", product.getProductId())
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void getProductById_NonExistentProduct_ReturnsNotFound() throws Exception {
    mockMvc
        .perform(get("/api/v1/products/{productId}", 9999L).contentType(MediaType.APPLICATION_JSON))
  ...
  void updateProduct_AsAdmin_ReturnsUpdatedProduct() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/products/{productId}", product.getProductId())
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void updateProduct_NonExistentProduct_ReturnsNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/products/{productId}", 9999L)
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isNotFound());
  ...
  void deleteProduct_AsAdmin_ReturnsSuccess() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/products/{productId}", product.getProductId())
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk());
